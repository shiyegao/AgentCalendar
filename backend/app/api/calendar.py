...
from ..models.calendar import CalendarEvent, Task, Category
...
def get_monthly_stats(year: int, month: int, db: Session = Depends(get_db)):
    ...
    avg_productivity = sum(event.productivity_score for event in events) / total_events if total_events > 0 else 0
